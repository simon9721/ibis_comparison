...
# Best-fit pad alignment from the clean simple-fixture RMS search.
# Negative means move the pybis waveform earlier in time.
PYBIS_SHIFT_NS = -0.607


class IbisWaveform:
    def __init__(self, kind: str, r_fix: float, v_fix: float, time_s: np.ndarray, v_typ: np.ndarray):
        self.kind = kind
        self.r_fix = r_fix
        self.v_fix = v_fix
        self.time_s = time_s
...
def parse_ibis_waveforms(path: Path):
    ...
    while i < len(lines):
        ...
        waveforms.append(IbisWaveform(kind, float(r_fix), float(v_fix), arr[:, 0], arr[:, 1]))
...
def main():
    ...
    time_pybis_shifted_ns = time_pybis_ns + PYBIS_SHIFT_NS
    time_ref_ns = ns(ref["time"])
    pybis_mask = time_pybis_ns <= t_max_ns + 0.001
    pybis_shifted_mask = time_pybis_shifted_ns <= t_max_ns + 0.001
    ...
    shifted_label = f"Converted pybis pad shifted {PYBIS_SHIFT_NS:+.3f} ns"
    shifted_load_label = f"Converted pybis load shifted {PYBIS_SHIFT_NS:+.3f} ns"

    fig, ax = plt.subplots()
    ax.plot(time_ref_ns[ref_mask], ref["v(pad_ref)"][ref_mask], label="Reference SPICE pad", linewidth=2.0)
    ax.plot(time_pybis_ns[pybis_mask], pybis["v(pad)"][pybis_mask], label="Converted pybis pad (original)", linewidth=1.4, linestyle="--", color="0.65")
    ax.plot(time_pybis_shifted_ns[pybis_shifted_mask], pybis["v(pad)"][pybis_shifted_mask], label=shifted_label, linewidth=2.0, linestyle="--", color="tab:orange")
    ...
    style_axis(ax, "Reference SPICE vs Shifted pybis vs IBIS Pad Overlay")
    ax.legend(loc="best")
    fig.tight_layout()
    fig.savefig(OUT_DIR / "refspice_vs_pybis_shifted_vs_ibis_rsf_pad.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
    ...
    ax_rise.plot(time_pybis_ns[pybis_mask], pybis["v(pad)"][pybis_mask], label="Converted pybis pad (original)", linewidth=1.4, linestyle="--", color="0.65")
    ax_rise.plot(time_pybis_shifted_ns[pybis_shifted_mask], pybis["v(pad)"][pybis_shifted_mask], label=shifted_label, linewidth=2.0, linestyle="--", color="tab:orange")
    ...
    style_axis(ax_rise, "Reference SPICE vs Shifted pybis vs IBIS Rise Zoom")
    ax_rise.legend(loc="best")
    fig_rise.tight_layout()
    fig_rise.savefig(OUT_DIR / "refspice_vs_pybis_shifted_vs_ibis_rsf_rise_zoom.png", dpi=180, bbox_inches="tight")
    plt.close(fig_rise)
    ...
    ax_fall.plot(time_pybis_ns[pybis_mask], pybis["v(pad)"][pybis_mask], label="Converted pybis pad (original)", linewidth=1.4, linestyle="--", color="0.65")
    ax_fall.plot(time_pybis_shifted_ns[pybis_shifted_mask], pybis["v(pad)"][pybis_shifted_mask], label=shifted_label, linewidth=2.0, linestyle="--", color="tab:orange")
    ...
    style_axis(ax_fall, "Reference SPICE vs Shifted pybis vs IBIS Fall Zoom")
    ax_fall.legend(loc="best")
    fig_fall.tight_layout()
    fig_fall.savefig(OUT_DIR / "refspice_vs_pybis_shifted_vs_ibis_rsf_fall_zoom.png", dpi=180, bbox_inches="tight")
    plt.close(fig_fall)
    ...
    ax_load.plot(time_pybis_ns[pybis_mask], pybis["v(ntst)"][pybis_mask], label="Converted pybis load (original)", linewidth=1.4, linestyle="--", color="0.65")
    ax_load.plot(time_pybis_shifted_ns[pybis_shifted_mask], pybis["v(ntst)"][pybis_shifted_mask], label=shifted_load_label, linewidth=2.0, linestyle="--", color="tab:orange")
    ax_load.plot(time_pybis_ns[pybis_mask], pybis["v(in_dig)"][pybis_mask], label="Input", linewidth=1.4, linestyle=":", color="gray")
    style_axis(ax_load, "Reference SPICE vs Shifted pybis Load Overlay")
    ax_load.legend(loc="best")
    fig_load.tight_layout()
    fig_load.savefig(OUT_DIR / "refspice_vs_pybis_shifted_rsf_load.png", dpi=180, bbox_inches="tight")
    plt.close(fig_load)

    print(f"Applied pybis shift: {PYBIS_SHIFT_NS:+.3f} ns")
    print(f"Saved: {OUT_DIR / 'refspice_vs_pybis_shifted_vs_ibis_rsf_pad.png'}")
    print(f"Saved: {OUT_DIR / 'refspice_vs_pybis_shifted_vs_ibis_rsf_rise_zoom.png'}")
    print(f"Saved: {OUT_DIR / 'refspice_vs_pybis_shifted_vs_ibis_rsf_fall_zoom.png'}")
    print(f"Saved: {OUT_DIR / 'refspice_vs_pybis_shifted_rsf_load.png'}")
