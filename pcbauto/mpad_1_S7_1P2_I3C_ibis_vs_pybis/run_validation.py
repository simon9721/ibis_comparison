...
CONVERTED_SUB = ROOT / f"{MODEL_NAME}-OutputInput-{CORNER}.sub"
CSV_PATH = ROOT / "validation_summary.csv"
MD_PATH = ROOT / "validation_summary.md"
PLOT_DIR = ROOT / "plots"
...
def bench_path_for(fixture: Fixture) -> Path:
    return ROOT / f"{MODEL_NAME}_fixture_1k_20pf_{fixture.label}.sp"


def raw_path_for(fixture: Fixture) -> Path:
    return ROOT / f"{MODEL_NAME}_fixture_1k_20pf_{fixture.label}.raw"


def plot_path_for(kind: str, fixture: Fixture) -> Path:
    return PLOT_DIR / f"{MODEL_NAME}_{kind}_{fixture.label}.png"
...
def plot_edge(sim_time_s: np.ndarray, sim_pad: np.ndarray, sim_in: np.ndarray, waveform: Waveform,
    ...
    ax.plot(edge_t_ns, sim_edge, label=f"pybis {edge_kind}", linewidth=2, linestyle="--")
    ...
    ax.set_title(f"{MODEL_NAME} {edge_kind} overlay ({fixture.label.replace('_', ' = ').replace('p', '.')})")
...
def main() -> int:
    ...
    for fixture in FIXTURES:
        ...
        write_bench(CONVERTED_SUB, subckt_name, vcc, enable_active_low, fixture, rise.c_fix)
    ...
    print(f"Wrote validation artifacts to {ROOT}")
    for row in rows:
        print(
