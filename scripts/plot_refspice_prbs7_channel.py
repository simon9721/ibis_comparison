...
RAW = ROOT / "ngspice_refspice" / "tb_refspice_prbs7_batch.raw"
OUT_DIR = ROOT / "plots" / "validation"
OUT_PNG = OUT_DIR / "refspice_prbs7_channel_waveforms.png"
...
def main():
    ...
    fig, ax = plt.subplots()
    ax.plot(t_ns, data["v(in_dig)"], "--", linewidth=1.2, label="Input v(in_dig)")
    ax.plot(t_ns, data["v(pad_ref)"], linewidth=1.6, label="Driver pad v(pad_ref)")
    ax.plot(t_ns, data["v(ntst_ref)"], linewidth=1.6, label="Load v(ntst_ref)")

    ax.set_title("Reference SPICE PRBS7 with 50 ohm T-line Channel")
