...
RAW = ROOT / "ngspice_refspice" / "tb_validation_compare_refspice_vs_pybis_rfr_batch.raw"
OUT_DIR = ROOT / "plots" / "validation"
EXPECTED_STOP_S = 26e-9
MIN_VALID_STOP_S = 24e-9
...
def require_complete_run(data):
    if len(data["time"]) == 0:
        raise RuntimeError("Incomplete raw for rise-fall-rise comparison: no usable transient points were written.")
    last_time = float(data["time"][-1])
    if last_time < MIN_VALID_STOP_S:
        raise RuntimeError(
            f"Incomplete raw for rise-fall-rise comparison: ended at {last_time * 1e9:.3f} ns, "
...
def main():
    ...
    style_axis(ax_pad, "Rise-Fall-Rise Pad Comparison: Reference SPICE vs Converted IBIS-SPICE")
    ax_pad.legend(loc="best")
    fig_pad.tight_layout()
    fig_pad.savefig(OUT_DIR / "refspice_vs_pybis_rfr_pad.png", dpi=180, bbox_inches="tight")
    plt.close(fig_pad)
    ...
    style_axis(ax_load, "Rise-Fall-Rise Load Comparison: Reference SPICE vs Converted IBIS-SPICE")
    ax_load.legend(loc="best")
    fig_load.tight_layout()
    fig_load.savefig(OUT_DIR / "refspice_vs_pybis_rfr_load.png", dpi=180, bbox_inches="tight")
    plt.close(fig_load)

    print(OUT_DIR / "refspice_vs_pybis_rfr_pad.png")
    print(OUT_DIR / "refspice_vs_pybis_rfr_load.png")
