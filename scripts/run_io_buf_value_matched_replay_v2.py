...
OUT_DIR = ROOT / "results" / "io_buf_value_matched_replay_v2_2026-06-26"
...
VARIANTS = [
    ...
    Variant("value_matched_v1", "ngspice value-matched v1 bug reference", "InputDrivenValueMatchedReplayHybrid", True),
    Variant("v2_balanced", "ngspice value-matched v2 balanced", "InputDrivenValueMatchedReplayV2Hybrid", True),
    Variant("v2_ku_only", "ngspice value-matched v2 Ku-only", "InputDrivenValueMatchedReplayV2KuOnly", True),
    Variant("v2_kd_only", "ngspice value-matched v2 Kd-only", "InputDrivenValueMatchedReplayV2KdOnly", True),
    Variant("v2_split", "ngspice value-matched v2 split Ku/Kd", "InputDrivenValueMatchedReplayV2SplitKuKd", True),
]


def build_cases(include_low: bool = False) -> list[StudyCase]:
    cases = [
        StudyCase("edge_1ps_base_50r_2pf", "Long-pulse control, 1 ps edges, 50 ohm + 2 pF", 0.001, 25.0, 50.0, 2.0, 3.3, "rise_fall"),
        StudyCase("short_pulse_500ps_high", "500 ps high pulse before output settles, 1 ps edges, 50 ohm + 2 pF", 0.001, 12.5, 50.0, 2.0, 3.3, "short_high", 0.5),
        ...
        StudyCase("double_toggle_1ps", "1 ps high then 1 ps low double toggle, 50 ohm + 2 pF", 0.001, 12.5, 50.0, 2.0, 3.3, "double_toggle", 0.001),
    ]
    if include_low:
        cases.extend(
            [
                StudyCase("short_pulse_500ps_low", "500 ps low pulse after high state, 1 ps edges, 50 ohm + 2 pF", 0.001, 18.0, 50.0, 2.0, 3.3, "short_low", 0.5),
                StudyCase("short_pulse_1ns_low", "1 ns low pulse after high state, 1 ps edges, 50 ohm + 2 pF", 0.001, 18.0, 50.0, 2.0, 3.3, "short_low", 1.0),
                StudyCase("short_pulse_2ns_low", "2 ns low pulse after high state, 1 ps edges, 50 ohm + 2 pF", 0.001, 18.0, 50.0, 2.0, 3.3, "short_low", 2.0),
            ]
        )
    return cases


def case_by_id(case_id: str, include_low: bool = True) -> StudyCase:
    cases = {case.case_id: case for case in build_cases(include_low=include_low)}
    if case_id not in cases:
        raise KeyError(case_id)
    return cases[case_id]


def selected_cases(case_ids: list[str], include_low: bool = False) -> list[StudyCase]:
    if not case_ids:
        return build_cases(include_low=include_low)
    return [case_by_id(case_id, include_low=True) for case_id in case_ids]
...
def build_pwl_points(case: StudyCase) -> list[tuple[float, float]]:
    ...
    if case.pattern == "double_toggle":
        fall_start = 5.0 + case.pulse_width_ns
        rise2_start = fall_start + case.pulse_width_ns
        return [
            (0.0, 0.0),
            (5.0, 0.0),
            (5.0 + e, hv),
    ...
    if case.pattern == "short_low":
        fall_start = 10.0
        rise_start = fall_start + case.pulse_width_ns
        return [
            (0.0, 0.0),
            (5.0, 0.0),
            (5.0 + e, hv),
...
def command_times(case: StudyCase) -> tuple[float, float]:
    ...
    if case.pattern == "double_toggle":
        return 5.0 + 0.5 * edge, 5.0 + case.pulse_width_ns + 0.5 * edge
    if case.pattern == "short_low":
        return 5.0 + 0.5 * edge, 10.0 + 0.5 * edge
...
def make_ngspice_deck(case: StudyCase, variant: Variant) -> str:
    ...
    if variant.save_diagnostics:
        extra = (
            ...
            " V(xdrv.kupre) V(xdrv.kdpre) V(xdrv.kupending) V(xdrv.kdpending) V(xdrv.hprehold)"
            " V(xdrv.hvmpending) V(xdrv.hvmholdtarget)"
            " V(xdrv.kusamp) V(xdrv.kdsamp)"
            " V(xdrv.tf_ku) V(xdrv.tf_kd) V(xdrv.tf_start)"
            " V(xdrv.tr_ku) V(xdrv.tr_kd) V(xdrv.tr_start)"
            " V(xdrv.vmstart) V(xdrv.vmstart_latch) V(xdrv.vmt0)"
            " V(xdrv.vmelapsed) V(xdrv.vmarg) V(xdrv.kuarg) V(xdrv.kdarg)"
            " V(xdrv.kustart_latch) V(xdrv.kdstart_latch)"
            " V(xdrv.match_err_ku) V(xdrv.match_err_kd)"
            " V(xdrv.start_disagree) V(xdrv.match_ambiguous) V(xdrv.hvmatch)"
            " V(xdrv.vmsample) V(xdrv.vmlatchpulse) V(xdrv.vmarg_backstep)"
            " V(xdrv.coeff_jump_ku) V(xdrv.coeff_jump_kd) V(xdrv.hnx)"
            " V(xdrv.hfall_after_rise) V(xdrv.hrise_after_fall) V(xdrv.hreverse_edge)"
            " V(xdrv.hintwindow) V(xdrv.had_rise) V(xdrv.had_fall)"
...
def status_for(case: StudyCase, row: dict[str, object]) -> str:
    ...
    if not all(np.isfinite([pad_rmse, ku_rmse, kd_rmse])):
        return "NUMERIC_FAIL"
    if str(row.get("coeff_range_ok", "True")).lower() == "false":
        return "NUMERIC_FAIL"
    if case.pattern == "rise_fall":
    ...
    if pad_rmse <= 0.025 and max(ku_rmse, kd_rmse) <= 0.03:
        return "GOOD"
    if pad_rmse <= 0.050 and max(ku_rmse, kd_rmse) > 0.03:
        return "PAD_ONLY_IMPROVEMENT"
    if str(row.get("flow", "")).startswith("ngspice_v2_"):
        vmarg_backstep = float(
            row.get("vmarg_match_active_max_negative_step", row.get("vmarg_max_negative_step", 0.0)) or 0.0
        )
        if vmarg_backstep <= 0.05:
            return "TIMER_BUG_FIXED_BUT_MODEL_BAD"
    return "CHECK"
...
def score_ngspice_row(case: StudyCase, variant: Variant, h_native: dict[str, np.ndarray], n_data: dict[str, np.ndarray], deck: Path, raw: Path) -> dict[str, object]:
    ...
    row: dict[str, object] = {
        ...
        "ngspice_row_count": int(len(n_t)),
        "ngspice_min_timestep_ns": float(np.min(np.diff(n_t))) if len(n_t) > 1 else float("nan"),
    }
    diag_signals: dict[str, np.ndarray] = {}
    for name in [
        "kusamp",
        "kdsamp",
        "kupre",
        ...
        if sig is None:
            continue
        diag_signals[name] = sig
        row[f"{name}_min"] = finite_min(sig, mask)
        row[f"{name}_max"] = finite_max(sig, mask)
        if name in {"vmarg", "kuarg", "kdarg"}:
            local = sig[mask]
            if len(local) > 1:
                row[f"{name}_max_negative_step"] = float(np.max(np.maximum(0.0, -np.diff(local))))
            else:
                row[f"{name}_max_negative_step"] = 0.0
        if name in {"hvmatch", "match_ambiguous"}:
            active = mask & (sig > 0.5)
            row[f"{name}_active_count"] = int(np.count_nonzero(active))
    if "hvmatch" in diag_signals:
        match_mask = mask & (diag_signals["hvmatch"] > 0.5)
        for name in ["vmarg", "kuarg", "kdarg"]:
            sig = diag_signals.get(name)
            if sig is None:
                continue
            local = sig[match_mask]
            row[f"{name}_match_active_max_negative_step"] = (
                float(np.max(np.maximum(0.0, -np.diff(local)))) if len(local) > 1 else 0.0
            )
...
def plot_case_figures(case: StudyCase) -> None:
    ...
    variant_specs = [
        ...
        ("value_matched_v1", "v1 value-match", COLORS["value_matched_v1"]),
        ("v2_balanced", "v2 balanced", COLORS["v2_balanced"]),
        ("v2_ku_only", "v2 Ku-only", COLORS["v2_ku_only"]),
        ("v2_kd_only", "v2 Kd-only", COLORS["v2_kd_only"]),
        ("v2_split", "v2 split", COLORS["v2_split"]),
    ]
    ...
    for variant_id, label, color in variant_specs:
        key = f"{variant_id}_pad"
        if key in data:
            ax.plot(t, data[key], color=color, lw=1.45 if variant_id != "v2_balanced" else 1.9, label=f"{label} pad")
    ...
    for coeff, ylabel, filename in [("ku", "Ku", "02_ku_overlay.png"), ("kd", "Kd", "03_kd_overlay.png")]:
        ...
        for variant_id, label, color in variant_specs:
            key = f"{variant_id}_{coeff}"
            if key in data:
                ax.plot(t, data[key], color=color, lw=1.45 if variant_id != "v2_balanced" else 1.9, label=f"{label} {ylabel}")
        mark_commands(ax, case)
        ax.set_xlim(*xlim)
    ...
    diag_prefix = "v2_balanced" if "v2_balanced_kusamp" in data else "value_matched_v1"
    diag_label = "v2 balanced" if diag_prefix == "v2_balanced" else "v1 value-match"
    fig, axes = plt.subplots(3, 1, figsize=(10.8, 8.0), sharex=True, constrained_layout=True)
    for ax in axes:
        mark_commands(ax, case)
        ax.set_xlim(*xlim)
    has_value_match_diag = f"{diag_prefix}_kusamp" in data
    if has_value_match_diag:
        axes[0].plot(t, data[f"{diag_prefix}_kusamp"], color=COLORS["diag_a"], lw=1.6, label="KUSAMP")
        axes[0].plot(t, data[f"{diag_prefix}_kdsamp"], color=COLORS["diag_b"], lw=1.6, label="KDSAMP")
        axes[0].plot(t, data.get(f"{diag_prefix}_kumatch", np.full_like(t, np.nan)), color=COLORS["v2_balanced"], lw=1.2, label="KUMATCH")
        axes[0].plot(t, data.get(f"{diag_prefix}_kdmatch", np.full_like(t, np.nan)), color=COLORS["hspice_transistor"], lw=1.2, label="KDMATCH")
        axes[1].plot(t, data[f"{diag_prefix}_tf_ku"], color=COLORS["diag_a"], lw=1.5, label="TF_KU")
        axes[1].plot(t, data[f"{diag_prefix}_tf_kd"], color=COLORS["diag_b"], lw=1.5, label="TF_KD")
        axes[1].plot(t, data[f"{diag_prefix}_tf_start"], color=COLORS["v2_balanced"], lw=1.7, label="TF_START")
        if f"{diag_prefix}_tr_start" in data:
            axes[1].plot(t, data[f"{diag_prefix}_tr_start"], color=COLORS["diag_c"], lw=1.3, label="TR_START")
        vmstart_key = f"{diag_prefix}_vmstart_latch" if f"{diag_prefix}_vmstart_latch" in data else f"{diag_prefix}_vmstart"
        if vmstart_key in data:
            axes[1].plot(t, data[vmstart_key], color=COLORS["diag_d"], lw=1.3, label="latched/active VMSTART")
        axes[2].plot(t, data[f"{diag_prefix}_match_err_ku"], color=COLORS["diag_a"], lw=1.4, label="MATCH_ERR_KU")
        axes[2].plot(t, data[f"{diag_prefix}_match_err_kd"], color=COLORS["diag_b"], lw=1.4, label="MATCH_ERR_KD")
        axes[2].plot(t, data[f"{diag_prefix}_start_disagree"], color=COLORS["diag_d"], lw=1.4, label="START_DISAGREE")
        axes[2].plot(t, data[f"{diag_prefix}_match_ambiguous"], color=COLORS["diag_c"], lw=1.4, label="MATCH_AMBIGUOUS")
    else:
        ...
        value_match_row = metric_lookup.get((case.case_id, "ngspice_v2_balanced"), {})
    ...
    fig.suptitle(f"{case.case_id}: value-match snapshot ({diag_label})", fontweight="bold")
    fig.savefig(out_dir / "04_value_match_snapshot.png", dpi=180)
    plt.close(fig)
    ...
    for ax in axes:
        mark_commands(ax, case)
        ax.set_xlim(*xlim)
    if "value_matched_v1_vmarg" in data:
        axes[0].plot(t, data.get("value_matched_v1_hnx", np.full_like(t, np.nan)), color=COLORS["diag_a"], lw=1.4, label="v1 HNX")
        axes[0].plot(t, data["value_matched_v1_vmarg"], color=COLORS["value_matched_v1"], lw=1.6, label="v1 VMARG")
    if "v2_balanced_vmarg" in data:
        axes[1].plot(t, data.get("v2_balanced_vmstart_latch", np.full_like(t, np.nan)), color=COLORS["diag_a"], lw=1.5, label="v2 VMSTART_LATCH")
        axes[1].plot(t, data.get("v2_balanced_vmelapsed", np.full_like(t, np.nan)), color=COLORS["diag_b"], lw=1.5, label="v2 VMELAPSED")
        axes[1].plot(t, data["v2_balanced_vmarg"], color=COLORS["v2_balanced"], lw=1.8, label="v2 VMARG")
    if "v2_split_kuarg" in data:
        axes[2].plot(t, data["v2_split_kuarg"], color=COLORS["v2_ku_only"], lw=1.5, label="v2 split KUARG")
        axes[2].plot(t, data["v2_split_kdarg"], color=COLORS["v2_kd_only"], lw=1.5, label="v2 split KDARG")
    elif "v2_balanced_vmlatchpulse" in data:
        axes[2].plot(t, data["v2_balanced_vmsample"], color=COLORS["diag_a"], lw=1.4, label="v2 sample pulse")
        axes[2].plot(t, data["v2_balanced_vmlatchpulse"], color=COLORS["diag_b"], lw=1.4, label="v2 latch pulse")
    style(axes[0], "v1 table time (ns)")
    style(axes[1], "v2 table time (ns)")
    style(axes[2], "split args / pulses")
    axes[2].set_xlabel("Time (ns)")
    for ax in axes:
        handles, labels = ax.get_legend_handles_labels()
        if handles:
            ax.legend(loc="best", ncol=3, frameon=False)
    fig.suptitle(f"{case.case_id}: timer diagnostics", fontweight="bold")
    fig.savefig(out_dir / "05_timer_diagnostics.png", dpi=180)
    plt.close(fig)
    ...
    metrics = [
        ...
        ("start_disagree_max", "Start disagree (ns)", 1.0),
        ("vmarg_match_active_max_negative_step", "active VMARG backstep (ns)", 1.0),
    ]
    flows = [spec[0] for spec in variant_specs]
    labels = [spec[1] for spec in variant_specs]
    colors = [spec[2] for spec in variant_specs]
    fig, axes = plt.subplots(len(metrics), 1, figsize=(11.2, 13.0), sharex=True, constrained_layout=True)
    ...
    fig.suptitle(f"{case.case_id}: value-match summary", fontweight="bold")
    fig.savefig(out_dir / "06_summary_bars.png", dpi=180)
    plt.close(fig)
...
def plot_summary(rows: list[dict[str, object]]) -> None:
    ...
    flows = [
        ...
        ("ngspice_value_matched_v1", "v1", COLORS["value_matched_v1"]),
        ("ngspice_v2_balanced", "v2 balanced", COLORS["v2_balanced"]),
        ("ngspice_v2_split", "v2 split", COLORS["v2_split"]),
    ]
        ...
        for idx, (flow, label, color) in enumerate(flows):
            ...
            for case in cases:
                ...
                try:
                    ...
                    values.append(np.nan)
            ax.bar(x + (idx - 1.0) * width, values, width=width, color=color, label=label)
        ax.set_ylabel(ylabel)
        ax.grid(True, axis="y", color="#d8dde6")
    ...
    fig.suptitle("Value-matched replay v2 summary", fontweight="bold")
...
def write_readme(rows: list[dict[str, object]], cache_rows: list[dict[str, object]]) -> None:
    ...
    control_v2_rmse = lookup.get(("edge_1ps_base_50r_2pf", "ngspice_v2_balanced"), {}).get("pad_active_rmse_v", "")
    try:
        control_delta_mv = (float(control_v2_rmse) - float(control_legacy_rmse)) * 1e3
    except (TypeError, ValueError):
        control_delta_mv = float("nan")
    ambiguous_cases = [
        f"{row['case_id']}:{row['flow']}"
        for row in rows
        if str(row.get("flow", "")).startswith("ngspice_v2_") and row.get("status") == "VALUE_MATCH_AMBIGUOUS"
    ]
    failed_cases = [
        f"{row['case_id']}:{row['flow']}"
        for row in rows
        if str(row.get("flow", "")).startswith("ngspice_v2_") and row.get("status") == "NUMERIC_FAIL"
    ]
    completed_2ns = [
        row.get("flow")
        for row in rows
        if row.get("case_id") == "short_pulse_2ns_high"
        and str(row.get("flow", "")).startswith("ngspice_v2_")
        and row.get("status") != "NUMERIC_FAIL"
    ]
    lines = [
        ...
        f"- Long-pulse v2 balanced pad RMSE delta versus legacy: `{control_delta_mv:.3f} mV`" if np.isfinite(control_delta_mv) else "- Long-pulse v2 balanced pad RMSE delta versus legacy: `n/a`",
        f"- V2 ambiguous rows: `{', '.join(ambiguous_cases) if ambiguous_cases else 'none'}`",
        f"- V2 numeric-fail rows: `{', '.join(failed_cases) if failed_cases else 'none'}`",
        f"- `short_pulse_2ns_high` completed v2 flows: `{', '.join(str(x) for x in completed_2ns) if completed_2ns else 'none'}`",
        ...
        f"- Legacy pybis pad peak: `{fnum('short_pulse_1ns_high', 'ngspice_legacy', 'pad_peak_v', 1.0, '.4f')} V`; v1 pad peak: `{fnum('short_pulse_1ns_high', 'ngspice_value_matched_v1', 'pad_peak_v', 1.0, '.4f')} V`; v2 balanced pad peak: `{fnum('short_pulse_1ns_high', 'ngspice_v2_balanced', 'pad_peak_v', 1.0, '.4f')} V`.",
        f"- HSPICE `Ku` peak: `{fnum('short_pulse_1ns_high', 'hspice_native_ibis', 'ku_peak', 1.0, '.4f')}`; legacy: `{fnum('short_pulse_1ns_high', 'ngspice_legacy', 'ku_peak', 1.0, '.4f')}`; v1: `{fnum('short_pulse_1ns_high', 'ngspice_value_matched_v1', 'ku_peak', 1.0, '.4f')}`; v2 balanced: `{fnum('short_pulse_1ns_high', 'ngspice_v2_balanced', 'ku_peak', 1.0, '.4f')}`.",
        f"- HSPICE `Kd` minimum: `{fnum('short_pulse_1ns_high', 'hspice_native_ibis', 'kd_min', 1.0, '.4f')}`; legacy: `{fnum('short_pulse_1ns_high', 'ngspice_legacy', 'kd_min', 1.0, '.4f')}`; v1: `{fnum('short_pulse_1ns_high', 'ngspice_value_matched_v1', 'kd_min', 1.0, '.4f')}`; v2 balanced: `{fnum('short_pulse_1ns_high', 'ngspice_v2_balanced', 'kd_min', 1.0, '.4f')}`.",
        f"- V2 balanced start disagreement: `{fnum('short_pulse_1ns_high', 'ngspice_v2_balanced', 'start_disagree_max', 1.0, '.3f')} ns`; v2 balanced match-active VMARG backstep: `{fnum('short_pulse_1ns_high', 'ngspice_v2_balanced', 'vmarg_match_active_max_negative_step', 1.0, '.3g')} ns`.",
        "",
        "## Key Files",
        "",
    ...
    for case in build_cases():
        for flow in [
            ...
            lines.append(
                "| {case} | {flow} | {status} | {pad_rmse} | {ku_rmse} | {kd_rmse} | {pad_peak} | {ku_peak} | {kd_min} | {start_disagree} | {vmarg_backstep} |".format(
                    case=case.case_id,
                    flow=flow,
                    ...
                    start_disagree=fnum(case.case_id, flow, "start_disagree_max", 1.0, ".3f"),
                    vmarg_backstep=fnum(case.case_id, flow, "vmarg_match_active_max_negative_step", 1.0, ".3g"),
                )
            )
    lines.extend(
        [
...
def verify_generated_models(model_paths: dict[str, Path]) -> None:
    text = model_paths["value_matched_v1"].read_text(encoding="utf-8", errors="replace")
    required = ["KUSAMP", "KDSAMP", "TF_START", "VMSTART", "MATCH_ERR_KU", "MATCH_ERR_KD"]
    missing = [token for token in required if token not in text]
    if missing:
        raise RuntimeError(f"Generated v1 value-matched model is missing diagnostics: {', '.join(missing)}")
    for variant_id in ["v2_balanced", "v2_ku_only", "v2_kd_only", "v2_split"]:
        v2_text = model_paths[variant_id].read_text(encoding="utf-8", errors="replace")
        required = ["KUSAMP", "KDSAMP", "VMSTART_LATCH", "VMELAPSED", "VMARG", "MATCH_ERR_KU", "MATCH_ERR_KD"]
        missing = [token for token in required if token not in v2_text]
        if missing:
            raise RuntimeError(f"Generated {variant_id} model is missing diagnostics: {', '.join(missing)}")
        if "VMARG 0 V = V(VMSTART) + V(HNX)" in v2_text:
            raise RuntimeError(f"Generated {variant_id} still uses the v1 VMARG/HNX expression")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Corrected io_buf short-pulse value-matched replay v2 experiment.")
    ...
    parser.add_argument("--include-low", action="store_true", help="Also run mirrored short-low optional cases.")
    parser.add_argument("--resume", action="store_true", help="Skip cases already present for all expected flows.")
...
def main() -> int:
    ...
    if args.summarize_only:
        ...
        for case in selected_cases(args.case, include_low=args.include_low):
    ...
    cases = selected_cases(args.case, include_low=args.include_low)
