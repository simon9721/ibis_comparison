...
def create_subcircuit_file_callback():
    ...
    subcircuit_type = radio_var1.get()
...
def create_all_models_subcircuit_file_callback():
    component_name = list_component.get(tk.ACTIVE)
    io_type = radio_var3.get()
    subcircuit_type = radio_var1.get()
    corner = radio_var2.get()

    logging.info("Create all models button pressed")

    if ibis_model is None:
        messagebox.showwarning(title="No IBIS file selected", message="Please browse and select a valid IBIS file first.")
        logging.error("No IBIS file selected")
        return

    if not component_name:
        messagebox.showwarning(title="No component selected", message="Please select a valid component first.")
        logging.error("No component selected")
        return

    output_dir = filedialog.askdirectory(parent=main_window)
    if not output_dir:
        return

    logging.info(f"Batch converting all models for component {component_name} into {output_dir}")
    results = subcircuit.generate_spice_models_for_all_models(ibis_model_ecdtools=ibis_model,
    ...
    if generated:
        message = f"Generated {len(generated)} SPICE subcircuit file(s) in:\n{output_dir}\n"
        if symbols:
            message += f"\nGenerated {len(symbols)} LTSpice symbol file(s).\n"
        if skipped:
            message += f"\nSkipped {len(skipped)} incompatible model(s):\n"
            message += "\n".join([f'- {item["model"]}: {item["reason"]}' for item in skipped[:12]])
            if len(skipped) > 12:
                message += f"\n... and {len(skipped) - 12} more"
        if failed:
            message += f"\n\nFailed {len(failed)} model/corner conversion(s):\n"
            message += "\n".join([f'- {item["model"]}: {item["reason"]}' for item in failed[:12]])
            if len(failed) > 12:
                message += f"\n... and {len(failed) - 12} more"
        messagebox.showinfo(title="Batch conversion complete", message=message)
        logging.info(message)
    else:
        message = f"No models were generated for component {component_name}."
        if skipped:
            message += "\nAll models were skipped because they were incompatible with the selected I/O type."
        if failed:
            message += "\nOne or more model conversions failed."
        messagebox.showwarning(title="Batch conversion produced no files", message=message)
...
if __name__ == '__main__':
    ...
    # Radio Buttons for subcircuit output type
    ...
    radio7 = tk.Radiobutton(master=frame3, text="NgSpice", variable=radio_var1, value="NgSpice")
    radio8 = tk.Radiobutton(master=frame3, text="Input-Driven", variable=radio_var1, value="InputDriven")
    radio8.select()  # Default to the ngspice input-driven flow
    radio1.place(x=170, y=10)
    radio2.place(x=250, y=10)
    radio7.place(x=335, y=10)
    radio8.place(x=425, y=10)
    ...
    ToolTip(radio7, msg="produces a generic ngspice-friendly subcircuit using pwl(...) syntax", delay=0.2)
    ToolTip(radio8, msg="produces an ngspice model with OUT/IN/EN/VCC/VSS pins and SPISim-style runtime Ku/Kd timing", delay=0.2)
    ...
    radio9 = tk.Radiobutton(master=frame3, text="Input", variable=radio_var3, value="Input")
    radio10 = tk.Radiobutton(master=frame3, text="Output", variable=radio_var3, value="Output")
    radio9.select()
    radio9.place(x=170, y=70)
    radio10.place(x=250, y=70)
    ...
    ToolTip(radio9, msg="subcircuit will be created for the input pin - no pullup/pulldown transistors", delay=0.2)
    ToolTip(radio10, msg="subcircuit will be created for the output pin - with pullup/pulldown transistors", delay=0.2)
    ...
    btn5 = tk.Button(master=frame3, text="Create All Models", command=create_all_models_subcircuit_file_callback)
    btn5.place(x=250, y=110)
    ToolTip(btn5, msg="converts every model in the selected IBIS file for the chosen component", delay=0.2)
